...
//        withUnsafeBytes(of: &bigEndian) { bytes.append(contentsOf: $0)}
        ...
        var dataBytes = Data()
        dataBytes.append(0x16)
        dataBytes.append(Data(msg.utf8))
        dataBytes.append(0x17)
        
        var bigEndian = crc.bigEndian
        withUnsafeBytes(of: &bigEndian) { dataBytes.append(contentsOf: $0)}
    ...
    func startGoOnChip(input: String, tags: String, tagsOpt: String) -> [UInt8] {
        ...
        print(String(bytes: msgGoOnChip, encoding: .ascii))
        return msgGoOnChip
    ...
    static func getFinishChip(input: String, inputTags: String, output: String) -> [UInt8] {
        
        let inputSize = String(format: "%03d", input.count)
        let inputTagsSize = String(format: "%03d", inputTags.count)
        let outputSize = String(format: "%03d", output.count)
        
        let inputFinishChip = PinpadCommands.finishChip.rawValue + inputSize + input + inputTagsSize + inputTags + outputSize + output
        
        print(inputFinishChip)
        
        let bytesTest: [UInt8] = []
